...
downloadWordNetFiles <- function(url="http://wordnetcode.princeton.edu/wn3.1.dict.tar.gz",dest=getwd(), verbose=TRUE){
  if(verbose){print(paste("Downloading dictionary files from ", url, sep=""))};
  destFile<-paste(dest, "wn.dict.tar.gz", sep="/");
  download.file(url, destFile);
  
  if(verbose){print("Unzipping dictionary files")};
  gunzip(destFile);
